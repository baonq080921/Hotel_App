<?php
    ...
    $roombookres = mysqli_query($conn, $roombooksql);
    $roombookrow = mysqli_num_rows($roombookres);

    // staff
    $staffsql ="Select * from staff";
    $staffres = mysqli_query($conn, $staffsql);
    $staffrow = mysqli_num_rows($staffres);

    // room
    $roomsql ="Select * from room";
    $roomres = mysqli_query($conn, $roomsql);
    $roomrow = mysqli_num_rows($roomres);

    //roombook roomtype
    $chartroom1 = "SELECT * FROM roombook WHERE RoomType='Superior Room'";
    $chartroom1res = mysqli_query($conn, $chartroom1);
    $chartroom1row = mysqli_num_rows($chartroom1res);

    $chartroom2 = "SELECT * FROM roombook WHERE RoomType='Deluxe Room'";
    $chartroom2res = mysqli_query($conn, $chartroom2);
    $chartroom2row = mysqli_num_rows($chartroom2res);

    $chartroom3 = "SELECT * FROM roombook WHERE RoomType='Guest House'";
    $chartroom3res = mysqli_query($conn, $chartroom3);
    $chartroom3row = mysqli_num_rows($chartroom3res);

    $chartroom4 = "SELECT * FROM roombook WHERE RoomType='Single Room'";
    $chartroom4res = mysqli_query($conn, $chartroom4);
    $chartroom4row = mysqli_num_rows($chartroom4res);
?>
<!-- moriss profit -->
<?php 	
